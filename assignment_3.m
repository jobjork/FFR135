...
iterations = 1e6;
num_runs = 10;
energy_train = zeros(iterations/1000, num_runs);
energy_val = zeros(iterations/1000, num_runs);
ct_err = zeros(num_runs, 1);
cv_err = zeros(num_runs, 1);
train_end = 1000;
c_err_t = zeros(train_end, run);
c_err_v = zeros(train_end, run);
...
for run = 1:num_runs
    ...
    for iter = 1:iterations
        ...
        if mod(iter,1000) == 0
            for i = 1:length(train_data)
                ...
                energy_train(iter/1000, run) = energy_train(iter/1000, run) + 0.5*(train_ans(i) - out_temp)^2;
            end
            
            for i = 1:length(val_data)
                out_val_temp = tanh( beta * (w'*val_pat(i,:)' - bias ));
                energy_val(iter/1000, run) = energy_val(iter/1000, run) + 0.5*(val_ans(i) - out_val_temp)^2;
            end
        end
        ...
        if iterations - iter < train_end
            ...
            c_err_t(ind_count, run) = 1/(2*length(train_ans))*sum(abs(train_ans - sign(c_err_t_temp(ind_count,:))'));
            c_err_v(ind_count, run) = 1/(2*length(val_ans))*sum(abs(val_ans - sign(c_err_v_temp(ind_count,:))'));
        end
    ...
    ct_err(run) = mean(c_err_t(:, run));
    cv_err(run) = mean(c_err_v(:,run));
end
...
plot(iter_vec,energy_train(1:10:end, :)/length(train_ans))
set(gca,'fontsize', 8)
...
axis([0 iterations 0.4 0.6])
iter_vec = linspace(1,iterations,iterations/(10*1000));
plot(iter_vec,energy_val(1:10:end, :)/length(val_ans))
set(gca,'fontsize', 8)
xlabel('Iteration', 'Interpreter', 'LaTex')
ylabel('Average Energy', 'Interpreter', 'LaTex')
set(gca,'fontsize', 14)
...
energy_val = zeros(iterations/1000,num_runs);
train_end = 1000;
c_err_t = zeros(train_end, num_runs);
c_err_v = zeros(train_end, num_runs);
ct_err = zeros(num_runs, 1);
cv_err = zeros(num_runs, 1);
    ...
    for iter = 1:iterations
        ...
        if mod(iter,1000) == 0
            for i = 1:length(train_data)
                ...
                energy_train(iter/1000, run) = energy_train(iter/1000, run) + 0.5*(train_ans(i) - O_temp)^2;
            end
            ...
            for i = 1:length(val_data)
                ...
                b_out_temp = w_out'*V_temp - bias_out;
                O_temp = tanh(beta*b_out_temp);
                energy_val(iter/1000, run) = energy_val(iter/1000, run) + 0.5*(val_ans(i) - O_temp)^2;
            end
        end
